    ...
    : EntityRepository<StaffReference>(factory, authorizationService), IStaffReferenceRepository
{
        ...
        CancellationToken cancellationToken, IDbTransaction? transaction = null)
    {
        ...
        const string sql =
            "SELECT [Id], [StaffMemberId], [ReferenceTypeId], [ReferenceStatusId], [RefereeName], " +
            "[RefereeOrganisation], [RefereeEmail], [RequestedDate], [ReceivedDate], [Notes], [IsDeleted], " +
            "[CreatedById], [CreatedByIpAddress], [CreatedAt], [LastModifiedById], [LastModifiedByIpAddress], " +
            "[LastModifiedAt], [Version] FROM [dbo].[StaffReferences] WHERE [StaffMemberId] = @staffMemberId " +
            "AND [IsDeleted] = 0;";
        ...
        try
        {
            ...
            return await conn.QueryAsync<StaffReference>(command);
        }
        finally
        {
            if (owns)
