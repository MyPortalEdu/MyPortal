...
-- ordering (Sun=0..Sat=6). Wrapped in sp_executesql so the parser doesn't reject batches run
-- against post-collapse schemas where the Weekday column no longer exists — column-name
-- resolution is immediate, not deferred, so the IF guard alone isn't enough.
IF EXISTS (
    SELECT 1 FROM sys.columns
